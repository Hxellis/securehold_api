...
iot.post("/updateLockerStatus", async (req, res) => {
    try {
        ...
        if (doorOpen) {
            ...
            const locationId = (await lockersModel.findOneAndUpdate({ _id: lockerId}, { $set: {door_status: false, last_used: Date.now()}, $inc: { usage_minutes: useTime}},  { new: true })).location
